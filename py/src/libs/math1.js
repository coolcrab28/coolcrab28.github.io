...
return float.$factory(Math.asin(float_check(x)))},
    ...
    cos : function(x){
        $B.check_nb_args('cos ', 1)
        $B.check_no_kw('cos ', x)
return float.$factory(Math.cos(float_check(x)))},
    cosh: function(x){
        $B.check_nb_args('cosh', 1)
...
return float.$factory(float_check(x) % float_check(y))},
    ...
    isclose:function(){
        $B.check_nb_args('isclose', 1)
        $B.check_no_kw('isclose', )
